...
% Finally, it was observed during all tests that the loss of a frame could be
% reported twice in the reception statistics: the last decoded frame number is
% only changed when a valid frame is parsed, and the frame sequence number is
% discarded if the frame contains too many erroneous bytes and cannot be
% corrected. While this avoid tracking corrupt frame sequence numbers in the
% decoder, it means that a corrupt frame will not be registered in the sequence,
% and the decoder will report a dropped frame when
...
nearly \SI{10}{\percent} of the system health packets transmitted by the
platform. The source of the problem could not be identified due to the lack of
time and the difficulty of debugging this subsystem in particular: the
Wemos D1 development board that was used as the \acrlong{obc} does not provide
useful printing devices that could have been used to send log messages.
