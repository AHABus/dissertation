...
Each payload is assigned its own task, with a running
interval depending on how the payload was configured before flight. Each task
fetches data from its payload, packetises it and adds it to the radio queue.
...
The \acrshort{gnss} task was designed to run at fixed intervals to parse the
\acrshort{gnss} receiver's output, and update the platform's cached
co-ordinates and altitude. This cached location is the one included in radio
packets.
