...
The \acrshort{ahabus} frame design was inspired by other frame-based protocols like the
...
\begin{figure}[H]
    ...
    \end{center}
    \centering
    \caption{\acrshort{ahabus} Frame Synchronisation State Machine}
    \label{fig:frame-fsm}
\end{figure}

Figure \ref{fig:frame-fsm} presents the finite state machine that
\acrshort{ahabus} frame decoders must implement to detect frames in a binary
stream, and figure \ref{fig:frame-fmt-3} the final version of the
\acrshort{ahabus} frame format.
...
explained further, the \acrshort{ahabus} frames were chosen to be 256-byte
...
\begin{figure}[H]
\includegraphics[width=0.40\textwidth]{dl-fldigi.png}
\centering
...
\begin{figure}[H]
\includegraphics[width=0.40\textwidth]{test-rig.jpg}
\centering
